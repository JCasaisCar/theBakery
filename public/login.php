<?php
    ...
    // Usamos la variable "$title" para asignar el título de la página a "login"
    $title = "login";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        // Verificamos si el resultado no es nulo y guardamos el valor
        if ($resultadoRol) {
        ...
        if ($resultado) {
            ...
            if (password_verify($password, $resultado['password'])) {
                echo ("<script>
                    ...
                    // Creamos una cookie con 'document.cookie'
                    document.cookie = 'username=$username; path=/';
                    document.cookie = 'rol=$rol; path=/';
                    // Verificamos si la cookie existe
                    if (getCookie('rol')) {
                        // Si el valor de la cookie es admin redirigimos a mainAdmin.php sino a mainCliente.php
                ...
                // Si la contraseña es incorrecta, mostramos un mensaje de error y ponemos un enlace para volver al login
                echo ("
            ...
            // Si el usuario no existe en la base de datos, mostramos un mensaje de error y ponemos un enlace para volver al login
            echo ("
            <div class='alert alert-danger mt-4' role='alert'>
